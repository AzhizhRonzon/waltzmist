...
import { CRUSH_HINTS, ALL_PROFILES } from "../context/WaltzStore";

interface SendCrushModalProps {
  onSend: (toId: string, hint: string) => boolean;
  onClose: () => void;
...
const SendCrushModal = ({ onSend, onClose, crushesRemaining }: SendCrushModalProps) => {
  ...
  const handleSend = () => {
    if (!selectedProfile || !selectedHint) return;
    const result = onSend(selectedProfile, selectedHint);
        ...
        {sent ? (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground font-body">
              They'll see a blurred card with your hint. Fingers crossed! 🤞
            </p>
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground font-body mb-1">
              {crushesRemaining} of 3 crushes remaining
            </p>

            {crushesRemaining <= 0 ? (
              <div className="glass rounded-2xl p-4 text-center mt-4">
                <p className="text-sm text-muted-foreground font-body">
                  You've used all 3 anonymous crushes. 💔
                </p>
              </div>
            ) : (
              <>
                <p className="text-xs text-muted-foreground/70 font-body mb-4">
                  Choose someone and a hint. Your identity stays hidden.
                </p>
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all text-left border ${
                          selectedProfile === p.id
                            ? "border-blossom/40 bg-blossom/10"
                            : "border-transparent glass hover:border-blossom/20"
                        }`}
                      >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={handleSend}
                  disabled={!selectedProfile || !selectedHint}
